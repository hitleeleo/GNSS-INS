...
public:
    ...
    double ref_alt, double ENU_LiDAR_Angle)
  {
    ...
    pub_debug_marker_ = nh_.advertise<visualization_msgs::MarkerArray>("/HDMap/LaneRoadsBoundaries", 1, true);
    ...
    */
    ...
    for (int i = 0; i < data.size() ; i++) // index all the buildings 
    {
      ...
      for (int j = 0; j < data[i].size() ; j++) // index the llh in one buildings
      {
        ...
        enu(0) = prex_ * cos(theta) - prey_ * sin(theta) ;
        enu(1) = prex_ * sin(theta) + prey_ * cos(theta) ; 
