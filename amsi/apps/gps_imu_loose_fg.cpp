...
bool gps_up = 0;
nav_msgs::Odometry odom_track,span_odom;
 ...
 /**
   ...
   * @return eWLSSolution 5 unknowns with two clock bias variables
    ...
    */
    ...
    odom_track.pose.pose.position.x = enu_wls(0);
    odom_track.pose.pose.position.y = enu_wls(1);
    odom_track.pose.pose.position.z = psr_cov;
...
int main(int argc, char* argv[])
{
  ...
  while (ros::ok()) { //ros::ok()
    ros::spinOnce();
    rate.sleep();
    GNSS_odom_pub.publish(odom_track);
    if ((imu_up == 1) && (online_cal_success ==1)) { // IMU measurement
     ...
     if ((gps_up == 1) && (online_cal_success ==1)) { // GPS measurement
     // if (imu_co > 20) { // imu  measurement
      ...
      double del_x_fix = span_odom.pose.pose.position.x - odom_track.pose.pose.position.x;
      double del_y_fix = span_odom.pose.pose.position.y - odom_track.pose.pose.position.y;
      double del_z_fix = span_odom.pose.pose.position.z - odom_track.pose.pose.position.z;
      ...
      fprintf(gps_imu_loose_fg_result, "%3.2f,%3.5f,%3.5f,%3.5f,%3.5f,%3.5f,%3.5f,%3.5f,%3.5f\n", output_time, 
            error_2D_fix,error_2D_fg, span_odom.pose.pose.position.x, span_odom.pose.pose.position.y, odom_track.pose.pose.position.x,odom_track.pose.pose.position.y,gtsam_position(0), gtsam_position(1)); // generateData_gt3    not always span_ecef available at the first epoch
