...
extern int outsolheads_Q(unsigned char *buff, const solopt_t *opt)
{
	const char *s1[] = { "WGS84", "Tokyo" }, *s2[] = { "ellipsoidal", "geodetic" };
	const char *s3[] = { "GPST", "UTC ", "JST " }, *sep = opt2sep(opt);
	char *p = (char *)buff;
	...
	if (opt->outhead) {
		...
		p += sprintf(p, ",Q=1:fix,2:float,3:3Dmap+sbas,4:dgps,5:single,6:3Dmap,ns=# of satellites)\n");
	}
	p += sprintf(p, "%s  %-*s%s", COMMENTH, (opt->timef ? 16 : 8) + timeu + 1, s3[opt->times], sep);
...
extern void outsolhead_Q(FILE *fp, const solopt_t *opt)
{
	unsigned char buff[MAXSOLMSG + 1];
	int n;
